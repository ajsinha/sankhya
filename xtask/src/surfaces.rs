...
const UNREACHED: &[(&str, &str)] = &[
    ...
    ("sankhya-sandbox", "M18, and the ordering is deliberate rather than incidental. ADR-0023 decides that a user-supplied function runs behind an operating-system boundary and that where the boundary cannot be built the feature is refused rather than degraded --- so the boundary is built and proved first, and `CREATE FUNCTION` is written against something that already exists. Built the other way round there is a window in which arbitrary Python runs with the server's identity, and windows like that are not closed later"),
