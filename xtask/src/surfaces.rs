...
const UNREACHED: &[(&str, &str)] = &[
    ...
    ("sankhya-clone", "M10, and design-gated on ADR-0016. This holds the vocabulary the rest of the milestone consumes --- what a clone records, and which tables a reclamation decision must consult instead of one. The clone action, the maintenance wiring and the refusals are the work that reaches it"),
