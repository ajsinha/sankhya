...
gate did not see any of this**: 2807 tests, 741 mutations and twenty checks, against silent data <!-- figures-as-measured-then -->
...
> **The self-comparison is closed; the scheduling is not, and cannot be here.** The soak's
> expectation now comes from the **writer**, accumulated as rows are written, rather than from a
> second read of the warehouse — a comparison of the published tier against itself agrees with
> itself, which is the one answer this check must not be able to give. It also flushes the
> fan-out accumulator first, because absorbed rows sit in a buffer until a partition is worth a
> file, and comparing without flushing compares *what was handed over* against *what was
> written*; the first run with the check switched on reported four tables missing rows for
> exactly that reason, in multiples of the batch size.
>
> That is sustained-soak evidence and **not** `M11`. `M11`'s criterion is reconciliation running
> clean *in production*, and this data was generated by the process checking it — a closed loop
> that cannot produce the source-side surprises the criterion exists to catch. The soak says so
> in its own comment rather than leaving the distinction to a reader.
...
> wrong number.
>
> `WriteStrategy::Mergeable` keeps its one real job — it drives the onboarding warning — and its
> doc comment no longer claims the other branch *"lets the storage layer skip merge machinery it
> will never need"*, which reads as though this branch has some.
>
> **The epoch is now a null.** A commit time lives on the transaction's `BEGIN` and a mutation
> does not carry one, so a non-nullable column forced the writer to invent a value, and
> `1970-01-01T00:00:00Z` is a wrong instant a reader cannot distinguish from a real one — a
> `WHERE _sankhya_commit_ts > …` excluded every row while looking like a filter that found
> nothing. The column is nullable, the writer writes null, and the assertion that used to
> require all three provenance columns non-null now distinguishes the two that order a row from
