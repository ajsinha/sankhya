...
| `NFR-PERF-08` | Streaming evaluation against materialized baselines plus bounded graph expansion | p99 < 50 ms at 500/s | **Not met and not claimed.** The graph tier answers since `M25`; nothing times it at 500/s, and the streaming half does not exist |
...
| `NFR-PERF-18` | The graph tier's freshness stated separately from the analytical tier's | Both published | **Not met.** An epoch carries the snapshot it was built from and publishes it as a column on every row; what is missing is a *freshness* figure, because nothing rebuilds an epoch after startup |
...
- **Graph hydration on a timer.** The engine is complete and, since 2026-09-14 (`M25`), **reachable**: a graph declared under `_graphs/` is hydrated at startup and held in a catalogue bound to the server, so `graph_reachable('payments', …)` answers. What is still absent is the *timer* — an epoch is built once, at startup, and never rebuilt, so a graph over a table that keeps arriving goes stale and nothing says so. Rebuilding on a maintenance tick is the next step, and `Overlay`/`RebuildThreshold` in `sankhya-graph` are the built, unwired half of it.
...
- **Bounds for the types a warehouse filters on arrived on 2026-09-14 (`M24a`) and not before.** `Date32`, every `Timestamp` unit and `Decimal128` were in the unhandled list, so a date-ranged query — most of them — pruned nothing at all, and the statistics that were recorded described the columns nobody filters on. They are recorded now, written to the log as the protocol spells them, and read back against the column's declared type. `Date64` is still absent, deliberately: it counts milliseconds while a date is a day, and folding one into a day count has to round a maximum *down*, which is the direction that skips a file holding rows.
...
| **M14** The client contract and the Python SDK | — | **In progress**, added 2026-08-31. The contract is decided in [ADR-0017](adr/0017-the-client-contract.md) and placed in [ARCHITECTURE](ARCHITECTURE.md) §11a. Python first; Java and Rust in M16, which is why the *contract* matters more than the binding. **This entry was rewritten on 2026-09-03**, because it had become three snapshots of the same list appended in reverse order --- *all four server-side prerequisites built*, then *three of the four*, then *two of the four*, with the older and now-false sentences reading as current. A reader taking the last paragraph at its word would conclude the principal was unbuilt, which is what happened. **All four server-side prerequisites are built.** (1) `SHOW LINEAGE OF` and `SHOW DEPENDENTS OF`, so a client can ask what a clone came from and what still reads a table *before* a drop refuses --- building them found that schema names were discarded at registration, that two tables of one name silently replaced each other, and that cloning had never worked against a table this server serves. (2) Refusals cross as data --- `code`, `sqlstate`, `remediation` and the `subjects` a refusal cites, in the protocol's hint field. (3) Version skew is refused at connection with both versions named, a client declaring no contract being served as the generic driver it is. (4) The principal travels: `Handler::query` and `Handler::visible_tables` had no parameter for one, so `authenticate` read the user, refused an empty one on the grounds that an unattributable connection cannot be audited, and then **discarded it** --- every statement authorized as a literal and every audit entry attributed to it. A `Caller` now carries the connection's startup parameters inward, and the audit chain's head differs when the same statement is run by two users, which is the property `FR-SEC-02` asks for. **The SDK's examples became tests 2026-09-02** --- twelve Python scripts and eight SQL scripts, both sets run against a live server on every build, and the SQL gate checks each statement's outcome in *both* directions, a `-- REFUSES` marker being required to fail and everything else to succeed. Writing them found four front-door defects a green gate had missed, and two shipped SQL examples containing statements that could never have run --- one carrying a written note claiming it had been verified against a live server, because a `psql` script with `ON_ERROR_STOP off` prints its errors and keeps going, so a wall of output reads as success. **Transport security built 2026-08-31**: one certificate, both doors, the wire protocol's own negotiation in front of it, and a startup line naming the posture in words. **What is left, precisely.** ~~*Authorization does not vary by subject*~~ **built 2026-09-03**: roles come from `server.users.<name>`, rules already grant by role, so this was the whole of the missing connection between a person and what they may read. **The presence of the map is the switch** --- an operator who has written down no users has not decided anything and gets the old behaviour, one role for everybody; an operator who has written down *one* has decided the list is the list, so a user absent from it holds no role. A separate flag would be a flag somebody forgets, and forgetting it in this direction grants access. Four tests, including the one that makes the others mean anything: a user whose role the policy has never heard of reads nothing, so `alice: reader` succeeding is not a server that ignores roles entirely. *Federated identity* (`FR-SEC-03`): mutual TLS puts a client's certificate where a door can see it and nothing derives an identity from it. ~~*TLS in the Python binding*~~ **built 2026-09-03**: `sslmode` takes PostgreSQL's own vocabulary --- `disable`, `prefer`, `require`, `verify-ca`, `verify-full` --- and `db.connection.encrypted` says what actually happened, so a caller asserts on their posture rather than assuming it. `require` against a server that declines **refuses**, which is the one behaviour here worth its own test: a client that asks for encryption, is told no, and continues has already sent the password it was protecting and cannot un-send it. The default is `prefer`, which is libpq's --- a binding that downgrades *silently* is what this repository refuses, and one that downgrades and reports it is a posture a caller can hold it to |
...
| **M17** Named snapshots | — | **In progress**, added 2026-09-01 by owner directive, ahead of M15 and M16. **Design gate met 2026-09-02**: [ADR-0019](adr/0019-named-snapshots.md) decides that a snapshot is a *position* rather than a table, that a table created after it is refused rather than answered as empty --- a table that did not exist is not a table that was empty --- that its expiry is mandatory, that it is not a permission cache, and that it is quoted as a session setting. Writing it exposed a live defect and fixed it: `SET` was accepted as a no-op, so `SET SNAPSHOT` would have served the present to a caller who asked for one instant. Settings that would change an answer are now refused by name. **Built 2026-09-02**: `sankhya-snapshot`, and `CREATE SNAPSHOT ... EXPIRE AFTER n DAYS`, `SHOW SNAPSHOTS` and `DROP SNAPSHOT` reaching it from a client, durable under `_snapshots/` so a snapshot survives a restart and a backup carries it. Wiring the pins found that **the running server told its sweeper nothing at all** --- `Maintainer::among` existed and only a soak test called it, so the maintenance thread ran with an empty lineage set, and an empty set pins nothing: a *clone's* files were reclaimable by the sweeper of the table they belong to. What still reads each table is now refreshed on the maintenance cadence and honoured by both. **Reading as of a snapshot is built**: `SET SNAPSHOT` resolves every table at the version the snapshot recorded, and a table it does not name is refused rather than answered from the present. **The git-tag half is built 2026-09-02** by owner directive --- `SHOW HISTORY OF <table>` lists every commit with what it did, whether the *writer* declared it a data change, and which snapshots and clones are keeping each version alive; `SET VERSION OF <table> = <n>` reads one table at one version. A row-level diff was explicitly excluded and parked in M20. Building it found five defects, four of them wrong answers rather than failures: a compaction *added* files declaring `dataChange: true` while removing them with `false`, so it read as a data change in one direction only; `SET VERSION OF` was swallowed by the generic `SET` no-op and served the present; a version beyond the log resolved to the newest, serving a version nobody has; a version whose files retirement had taken was answered short; and `kept_by` said the word `snapshot` rather than naming one, and never mentioned a clone. **Exit criteria written and demonstrated 2026-09-02** through the shipped binary and the wire protocol. Writing criterion 2 found that nothing had ever demonstrated the property the feature exists for: the test that read as of a snapshot moved **one** table, which proves the setting is honoured and says nothing about many tables sharing one instant --- a snapshot that pinned each table at whatever version it reached would be a clone with extra steps, and one table cannot tell the two apart. Two tables now move by different amounts after the snapshot, are read in one session, and are joined across --- because a mixed instant does its real damage in a join, where it returns rows that look like an answer. A *name* for a consistent position across many tables, pinned so its files stay alive. A clone pins one table at one version; a market-risk run reads trades, rates, curves and hierarchy and must read all of them as of one instant, or the reconciliation problem reappears inside a single query. Nearly free: the read path already splices against a target position, and nothing is copied |
| **M21** The built-in function catalogue | — | **Design gate met 2026-09-02**, added by owner directive the same day: the catalogue must be *very wide* --- linear algebra, matrices, calculus, statistics, vector mathematics, Excel-style and graph functions --- reachable from **both** the SQL surface and every SDK, and vectorized. [ADR-0020](adr/0020-the-built-in-function-catalogue.md) decides six things, two of which were not obvious. **A statement calling a built-in is an analytical statement**: the transactional tier is PostgreSQL and a Rust UDF does not exist there, so the router adapts rather than the user --- a function is never refused or answered differently because of a routing decision nobody can see, and implementing each function twice was refused because two implementations disagree eventually and the day they do the answer depends on which tier served it. **And the speed does not come from SIMD.** Lane-parallel accumulation is 10--15× faster and computes a different, worse number: on `1e16, 1, -1e16, 1` repeated, whose exact total is 18, it returns 5 --- and 0 when the input is reversed. A conditioning threshold to fast-path only safe inputs never fell back across 200,000 randomised vectors and still differed from the exact total in 57% of them. The answer was in `sankhya-math`'s own documentation: fixed-point accumulation, where integer addition **is** associative, so order-independence holds by construction rather than by sorting. `exact_sum` is bit-identical to the sorted sum across 200,000 randomised vectors, unchanged under permutation in every one, and the fixed-point route costs **3.3× / 3.0× / 3.0×** less than the sorted-expansion fallback at 64, 512 and 4,096 values ([bench: sankhya-math/deterministic-sum]) --- a cost-of-route comparison, since the fallback runs only where the fixed-point route declines and the two arms therefore sum different numbers. This entry said "1.3× to 3.6× faster through the shipping kernels" until 2026-09-06; that is the per-kernel figure `ADR-0020` **retracts**, because its "before" is the sorted-only implementation and no longer exists, so nobody can re-run it. The SIMD figure beside it is unmeasurable here for the same class of reason [rejected: the lane-parallel kernel is not in this build, so nothing here can reproduce the figure; it is stated because a reader who does not know it will propose it]. **Ten kernels were found already written, tested and unreachable from SQL** --- `differences`, `derivative`, `second_derivative`, two integrators, two cumulative forms, `range`, `standardise` and `linear_fit`, the last being linear regression. The recurring failure of this repository, found again |
| **M21a** Vectors, arrays and matrices as column types | — | **Design gate met 2026-09-02**, [ADR-0021](adr/0021-vectors-matrices-across-the-tiers.md), after the owner asked whether a column can be declared a vector on **both** tiers. It can, and the analytical half is **demonstrated end to end**: a `FixedSizeList<Float64,3>` column published through the product's own writer, stored in Parquet, read back, and ranked by `ORDER BY vec_cosine_similarity(embedding, …)`. The width survives, because the Delta schema carries a `sankhya.fixedLength` key --- which `ADR-0021` Decision 2 makes part of the type, since it is what lets a kernel take a contiguous slice instead of copying per row. The PostgreSQL half needs no extension for arrays (`float8[]` is native); `pgvector` is the candidate for a typed width and an index, and Decision 5 settles what an index may do: **an index narrows candidates and only a kernel reports a distance**, because an approximate index that ranks and a function that scores are the two-implementations problem again. **Writing the demonstration found that the write path enforced no schema at all**: `publish_table` compared a batch against the schema it was *handed*, and `append` --- the path feeds, compaction and every test use --- compared nothing, so a table declaring `id, amount` accepted a batch of two entirely different columns and a width-3 vector column accepted a width-4 vector. Now checked against the table's own metadata, by `ARCHITECTURE` §6.6's rule: additive changes pass, contradictions are refused. The first thing the check caught was a **false positive that was itself a finding** --- the quarantine table writes UTC-aware timestamps into a column its metadata calls naive --- and the fix is that the comparison asks what the *format records* rather than what Arrow holds, because the round trip erases the zone before any reader sees it. **And a vector was crossing the wire as `text`** --- a client received the characters `[1.0, 2.0]` and had to parse them, which fails on a null element, on a locale rendering a decimal comma, and on an empty array against a null one. It is now `float8[]`, OID 1022, rendered `{1,2.5,3}`: the type every PostgreSQL driver already decodes, so the correct thing also deletes code from every client. The OID and the rendering are one change and neither is right without the other --- announcing the type while sending Arrow's brackets would be worse than sending text, because a client would then try to decode them |
| **M21b** The catalogue, and both wirings | — | **Built 2026-09-02.** The gap that caused both: *"which functions does this server have"* was answerable only by reading Rust, so no binding could generate them and no router could classify a statement. `SELECT * FROM functions()` now serves the catalogue as data --- name, category, arity, argument shape, return shape and a sentence --- over **every** function a session registers, not one crate's, because a catalogue that is wrong about the thing it describes is worse than none. Two tests hold it: the entries and the registrations must agree in **both** directions (an entry for a function nobody registered becomes a binding method that does not plan), and the completeness is asserted against a **real session**, which is the only place the whole set is visible. **The SDK gap is closed**: `db.fn.<name>(...)` reads the catalogue once per connection and offers all 128, with no stub written per function --- a function added to the server appears in the binding with no change to the binding. It stays thin: it validates nothing, so a wrong argument gets the server's own refusal; a **typo in a name** it answers itself, because that needs no round trip. **The OLTP rule is built before the router**, which is the point: `tier_for` reports that a statement naming a built-in is an analytical statement, and it is unaffordable to retrofit --- by the time both tiers answer, the second implementation of every function is already written. It is also the half checkable without a cluster, and the half that will be wrong: `erfc(x)` must not read as a call to `erf`, a column called `erf` must not route a lookup analytically, and `norm_cdf (x)` with a space must still be a call. The match is deliberately eager, because a literal routed the slow way costs one query and a missed call is a refusal for a function the catalogue says exists |
| **M21c** The parity soak, and computing where the data is | — | **Built 2026-09-02.** The requirement stated plainly: *a user does not care whether an answer came from OLTP, OLAP or the SDK, and the experience must not depend on it.* A unit test cannot reach that --- it asks whether **one** path is right, and three paths can each be right about a different thing. So the soak asks the only question that captures it, **do the paths agree**, of every function in the server's own catalogue, with arguments generated from what the catalogue says each takes: 110 of 128 covered, 1,540 comparisons a pass, and every one of the 18 gaps named with its reason, because a soak that covers 110 and prints PASS has made a claim about 18 it never ran. It found eight functions where every path refused identically --- **consistent refusal is still refusal**, and it hid three real defects: `mat_is_square` was registered through the numeric wrapper and so refused the array it exists to be asked about; `vec_quantile` mapped every failure onto `Empty`, so a probability outside `[0, 1]` was reported as an empty vector and sent somebody to look at their data instead of their argument; and **the catalogue had two matrix conventions** --- `mat_cholesky(vec_of(...))` worked and `mat_determinant(vec_of(...))` did not, because the older functions require a shape declared by `mat_of`. Square-only operations now deduce their order from the length, which is a deduction rather than a guess since any other shape is invalid for them; rectangular ones still refuse, because sixteen values are a 4x4 or a 2x8 and transposing the wrong one produces numbers from values that were never in the same row. **And the reason for the catalogue is now measured rather than asserted**: the same twelve value-at-risk numbers cost **500 bytes computed in the warehouse and 14,687 fetched to the client, 29x**, with both routes asserted to agree so it is a comparison of cost rather than of two answers. The fixture grew a table of profit-and-loss vectors to make that possible --- every example until then called a function on a literal carried from the client, which is precisely what the catalogue exists to make unnecessary, and a fixture with no vector column is why nobody noticed. A vector column also reported its type as `text` in `information_schema`, while the same column crossed the wire as `float8[]`: two answers about one column, and the catalogue's was the wrong one |
| **M21d** Designed for scale, and the wrappers made to borrow | — | **Owner directive 2026-09-02**: the library is meant to grow until it *rivals MATLAB*, designed carefully and written for high performance. [ADR-0020](adr/0020-the-built-in-function-catalogue.md) Decision 7 records what breaks at that size --- one file per category stops working at about fifteen functions each against `check-loc`'s cap, one crate becomes the compile bottleneck every build waits on, and hand-written registration stops scaling before the functions do, with the catalogue entry beside each a second copy that drifts. The drift test written for the current shape is what makes the migration to a generated table safe. **And the performance claim was false in my own new code**: I criticised the old wrappers for allocating a `Vec` per argument per row, then wrote four new wrappers that did exactly the same thing. A `FixedSizeList` stores its rows end to end, so a row is a borrowed slice with a known stride --- measured at **14.3x** at width 8, 5.1x at 64 and 1.5x at 512, with identical results --- by `crates/sankhya-functions/benches/rows.rs`, which is a build target. `PERF-01`: this line used to say 24.9x / 7.2x / 2.1x and nothing in this repository ever produced those figures; the fast arm of whatever produced them was almost certainly deleted by the optimiser, because its result was unused. Three wrappers now borrow; the fourth holds several arrays at once and reuses its buffers instead, which removes the allocation without contorting the API. Two smaller corrections came with it. A defensive line carried a slice offset with a comment calling it a hazard, and the premise was **false** --- arrow slices a `FixedSizeList`'s child values too, so there is no offset --- and the line is gone rather than kept with a note, because a defensive line whose premise is false reads as evidence somebody checked. And the test for the buffer reuse **could not fail** through two versions: the first called a function belonging to a different wrapper, the second used `UNION ALL`, where each branch is its own batch so the buffers were rebuilt between rows and never accumulated. It needed several rows in one batch and three different slopes, because identical slopes fit the same line however the rows are run together |
| **M21e** Time series, finance and risk; and the soak reaching a column | — | **Built 2026-09-02**, taking the catalogue to **155** SANKHYA functions: rolling statistics, exponential smoothing, simple and logarithmic returns, drawdown, autocorrelation, net present value under both conventions, internal rate of return, annuities, depreciation, historical value-at-risk, expected shortfall, Sharpe, Sortino, Black-Scholes and two greeks. Sixteen tests, including put-call parity and an internal rate verified by its own defining property; **three of my expected values were wrong and the implementations right**, one of them a Black-Scholes price written from memory. Nine mutations, all caught. **And the soak now reaches a column**, which was the gap: a function called on a literal is a scalar broadcast, and the same function over a column is an Arrow array read by stride --- the path a real query takes, and until now the one the soak did not exercise. It calls **94 functions over `risk.positions.pnl`** and checks each against the same function called on a literal built from that row's values: same values, same function, two entirely different routes into the kernel. That found a refusal telling somebody their 8x8 matrix **is not square** --- `cholesky` reused `NotSquare` for a symmetry failure, so the message contradicted itself and sent the reader to check a shape that was correct. Sixty-four is a square number, so one column exercises the series, series-and-number, two-series and matrix shapes without four fixtures that could drift |
| **M21f** Every function called, on every path | — | **Built 2026-09-02** by owner directive: *thoroughly test each and every function in OLTP, OLAP and the SDK, over an actual table column as well as over numbers written in SQL.* The soak now compares **144 of the 155** on three paths and reports the other eleven by name with the reason --- nine need a declared cube or graph, and two are table functions, which return rows rather than a value. The figure that mattered was a different one: **the number of functions that refused on every path**, which is agreement, and which means their *answers* were never compared once. It was twenty-six, and it is now **zero**. Driving it there is what found the arguments the generator had wrong --- a Sortino ratio over a series that never fell, a logarithmic return over prices that went negative, a depreciation period outside the asset's life --- and three defects. **A matrix column lost its shape when it was stored**: the Delta writer kept one key of its own and dropped every other key the column declared, so a stored matrix came back not being a matrix. It had no symptom worth noticing --- the table read perfectly, every function that deduces a square order answered, and only `mat_transpose`, `mat_multiply` and `mat_vec` refused, which reads as three functions being awkward rather than as a storage defect (ADR-0021 Decision 4b). **The binding could not write a whole number**: it rendered every `int` as a float, so `db.fn.ts_rolling_mean(p, 5)` was refused for a window that must be whole while the identical statement in `psql` answered --- the same unasked-for coercion the branch above it refuses to make for a boolean. And **Simpson's rule refused an even sample count by saying it 'cannot combine vectors of length 64 and 65'**, about a second vector the caller never passed; the test that covered it read the two fields of the error and never rendered the message. The binding gained a `matrix(rows, columns, values)` argument type, which is what makes the whole matrix family reachable from `db.fn` at all, and the fixture gained a stored 4x4 whose column declares its shape. **2304 comparisons per pass, 150 probes over stored columns** |
| **M18a** A cube over a declared query | — | **Built 2026-09-02**, the first half of M18 and the prerequisite [ADR-0014](adr/0014-materialized-views-and-the-cube-lifetime.md) had been waiting on since 2026-08-28. `CREATE CUBE <name> FROM ( <query> )`: the facts may be a join, a filter or a projection rather than a published table's name, so a user wanting a cube over a join no longer has to publish the join as a second table and keep it fresh themselves. **Nothing else about a cube changed**, which is the whole design: [ADR-0012](adr/0012-open-capabilities.md)'s rule is that an artefact which cannot say what it needs cannot be cached correctly, checked against policy, or bounded --- and a query's *text* does not say what it needs, because `FROM orders` names a different table under a different search path and a name inside a string literal is not a reference at all. So the query is **planned once, under the caller's own guard**, and the tables it turns out to read are recorded with the definition. Those tables are then exactly what the fact table already was: the authorization check, the snapshot the cache is keyed on, and part of the fingerprint. A cube over a join is keyed on the **newest** of its inputs, so a commit to either side is a miss; and it is authorized against **both**, because a caller allowed to read one side is not allowed to read what the join makes of the pair. Two refusals land at declaration rather than later: a query that cannot be planned has no dependency list, and a cube with none would be checked against nothing and invalidated by nothing; and a query whose answer can move on its own --- `now()`, `random()`, `current_date` --- is refused naming what it found, because a cuboid built from it is a cache of one arbitrary answer. Proved end to end over the wire: the cube's roll-up **totals what its own fact query totals**, which is the only assertion that could fail for the right reason --- a cube reading only `orders` would answer too, with a number nobody could tell apart by looking. Writing it caught a weak assertion of my own: the dependency list was checked with `contains("orders")`, and the fallback when nothing is persisted is the query text, which *contains* the word `orders` --- so the test passed for a cube that had recorded no dependencies. Asserted as the exact names, the mutation is caught |
| **M18b** The boundary a user function runs behind | — | **Decided and built 2026-09-03**, and built *before* the surface that will use it, deliberately. [ADR-0022](adr/0022-user-defined-functions.md) left sandboxing open by name; shipping `CREATE FUNCTION` first would have opened a window in which any principal who can issue DDL runs arbitrary code as the server user with a principal's filtered rows in hand, and windows like that are not closed later. [ADR-0023](adr/0023-the-sandbox-a-user-function-runs-in.md) decides it: **the isolation is enforced by the kernel, outside the process running the code**, and restricting Python from inside Python is refused by name --- not as a weak boundary but as a *decoration*, which is worse than none, because with no boundary nobody grants the capability lightly. `sankhya-sandbox` is that boundary: a network namespace with no route, a mount namespace pivoted onto a read-only tree holding the interpreter and nothing else, `PR_SET_NO_NEW_PRIVS`, a file-size limit of zero, an address-space and CPU bound, a wall-clock deadline the parent enforces by killing, and a cap on the bytes returned. **Eight tests, each of which tries the forbidden thing** --- opens a socket, reads a file it was not given, writes, loops forever --- because a test asserting which flags were passed would pass for a mechanism this kernel does not honour, which is the case worth detecting. Five mutations, all caught; there is deliberately none for the file-size limit, because writing is stopped twice and removing either mechanism alone leaves it stopped. Building it corrected two things the design had wrong. **A `seccomp` filter cannot deliver *no subprocess*** --- the mechanisms are applied between `fork` and `exec` and the child must `exec` once to become the worker, so a filter denying `execve` denies that one; what actually delivers the prohibition is that the jail is empty. And **`sankhya-alloc` had held the *only* unsafe-code exception since it was written**, in a manifest and a module header that both said so; a second was now needed, and the sentence had quietly stopped being true. `check-unsafety` now holds the list in one place and fails the build for a crate that opts out and is not on it --- and it fails for a *permitted* crate that no longer writes `unsafe`, so a permission cannot outlive its reason. The crate is listed in `UNREACHED` with this milestone, which is the honest state: the boundary exists and nothing calls it yet |
| **M18c** An aggregation of your own | — | **Built 2026-09-03** by owner directive --- *user supplied merge functions capability can be very useful for custom cubing, please build it.* `CREATE AGGREGATION <name> LANGUAGE PYTHON AS $$ … $$`, and it is reachable: `SHOW AGGREGATIONS` lists it with its source, `DROP AGGREGATION` removes it, it survives a restart, and it is callable in any `GROUP BY` from `psql`, from `db.sql`, and from the binding's own `create_aggregation`. Four methods, which are [ADR-0010](adr/0010-external-aggregations.md)'s contract --- and they are **DataFusion's `Accumulator`, method for method**, which decided the design: a supplied aggregation is an ordinary `AggregateUDF`, so it partitions and recombines exactly as a built-in does and a cube measure naming one will be computed by the same machinery rather than by a second path. **A declared `merge` is the claim that partial results compose**, and the claim is checked: the same values accumulated one way and several ways, merged in two groupings, compared **bit for bit**. Writing the mutation for that comparison found the test could not fail --- no fixture differed by less than a tolerance would allow --- so there is now a merge that is associative to twelve places and not to sixteen, which is the case a tolerance would have accepted and which surfaces as a total that does not match its own subtotals in the last penny. The same happened to the null test: a missing *weight* reads as zero and leaves the answer unchanged, so the fixture needed a missing *value* as well, which would have turned 17.5 into 70/9. Three refusals land at declaration --- an answer that depends on batching, a merge that is not associative, and a state that is not JSON and therefore not materialisable, which `ADR-0010` requires be said at declaration rather than discovered when a cuboid fails to write. It runs behind M18b's boundary; where that cannot be built it is refused naming the mechanism. **`ADR-0010`'s *Arrow in, Arrow out* is amended**: reading Arrow IPC in Python needs `pyarrow`, which would make the feature unavailable --- and untestable here --- on a machine that has Python and not that package, so what crosses is Arrow's own buffer layout without the envelope, which `numpy.frombuffer` reads at no copy and the standard library reads as a `memoryview`. The property the decision wanted, a boundary crossed per batch, is kept exactly. And the dispatch taught its own lesson: `claims()` carries a comment saying that answering *which statements are ours* in two places is how the next one comes to be shadowed silently --- and `SHOW AGGREGATIONS` was answered by the catalogue as an unknown setting until it was added there too |
| **M18d** A cube measure of your own | — | **Built 2026-09-03**, the half of the owner's directive that says *for custom cubing*. `MEASURE margin_pct (AGGREGATION rms ALONG region)`: a measure may name a declared aggregation as its rule along a dimension, and the cube answers with it. A root mean square is not a sum, a last, a max, a min or a mean, so before this there was no way to declare it as a measure at all --- and `MEAN` is refused along any dimension for the reason that makes this worth having. Proved against the same arithmetic written out in SQL, which is the only assertion that could fail for the right reason. **A roll-up recomputes over the facts**, not over partials: rolling a dimension away carries its cells' contributions up whole, so the answer at a coarse grain is the answer the function would have given at the base grain over the same rows, and `merge` matters for a *materialised* cuboid where partials are all that survive. Whether a measure composes is taken from the **declaration** and never from the cube --- a `CREATE CUBE` that could assert it would let a cube claim a property its function does not have. Writing the mutation for the roll-up found the test could not fail: the cube had one dimension, and with one dimension every query is already the base grain, so the code that combines a user's aggregation across cells never ran. It has two now. `Rule` stays `Copy` --- the aggregation's name lives on `Along` beside it, because a `Rule` carrying an owned string would cost an allocation in every corner of the cube machinery for a variant almost no cube uses --- and a `Supplied` rule with no name is refused by validation, since the two fields are one fact. Stated limitation: **a cube measure is one column**, so a cube rule sees one value per fact; a weighted mean needs two and stays a query-level aggregate until a measure can name more than one column |
| **M18e** A derived result | — | **Built 2026-09-03**, and it **accepts [ADR-0014](adr/0014-materialized-views-and-the-cube-lifetime.md)**, Proposed since 2026-08-28. `CREATE DERIVED <name> FROM ( <query> )`: a query given a name, selected from like a table, listed by `derived()`, dropped by `DROP DERIVED`. Option A, exactly as the ADR wrote it --- **a definition with no dimensions and no measures is simply a maintained query** --- and the shape is the discriminator rather than a flag beside it, because a flag can disagree with the shape and this cannot. Everything the ADR worried about duplicating is reused unchanged: the declared query, the dependency list resolved by planning under the caller's guard, the snapshot key, the fingerprint, the catalogue. There is no second refresh loop and no second staleness rule because there is no second implementation, which was the ADR's whole argument --- *this warehouse has already paid for that twice.* A separate **statement** from `CREATE CUBE` for a smaller reason: `validate` refuses a cube with no dimensions and gives useful advice, and that advice would become a lie if one statement meant two things. **The maintained lifetime is refused by name**, and that refusal is the most load-bearing line in the slice: `MAINTAINED WITHIN n VERSIONS` parses and sets a target lag, and the maintenance loop walks a cube's *measures* to decide what to build --- a derived result has none, so it would build nothing while `derived()` reported its lifetime as `maintained`. A clause accepted and ignored is worse than a clause refused and worse than one that does not parse, because the operator has been *told* their staleness bound is honoured. A cube and a derived result share a namespace, so `DROP CUBE` naming a derived result is refused and reports it absent rather than confirming it exists. One mutation survived at first --- the refusal of a derived result over a bare table name --- because the *parser* rejects it before validation is reached; the guard is for the other way in, a definition read back from a catalogue, and it now has a test at that level |
| **Docs** The documentation is executed | — | **Built 2026-09-03** after the owner named documentation rot twice in one day. `check-doc-numbers` catches a stale figure and `check-docs` catches a stale status line; neither catches **a sentence that stopped being true**, and three of those surfaced in one morning --- `sankhya-alloc` claiming to be the only crate permitted `unsafe`, ADR-0021 claiming a `FixedSizeList` cannot carry tensor metadata through Parquet, and Chapter 19 documenting a fixed `MEAN`/`MAX` defect as live, *with a transcript*. `tests/book_sql.rs` now extracts every SQL block in `docs/`, runs each statement against a live server, and checks the outcome **in both directions**: shown as working must work, shown as refused --- the book's own `-- ERROR:` convention --- must be refused. The line is drawn narrowly and stated: *a statement shown as working must fail only because the object it names is absent*, since much of the book is a transcript from a warehouse this fixture is not. **127 statements run, 44 excused as naming absent objects, 18 syntax diagrams skipped.** Its first run found six things and four of them were defects in the check itself --- trailing `--` comments merging two statements into one, newlines collapsed inside `$$ … $$` where Python's indentation is load-bearing, a transcript's reply read as a heading for the next statement, and clauses like `WHERE …` run as statements. The other two were the book: `mat_of(2, 3, …)` shown with five values where a 2x3 needs six, and a cube example naming an aggregation the chapter never declared. That last one was written that morning, in a section documenting a feature built that morning, and caught by a check written that morning. **Documentation rot does not need time.** And the M14 entry in this very file was rewritten the same day for the same reason: it had become three snapshots of one list appended in reverse order --- *all four prerequisites built*, then *three of the four*, then *two of the four* --- with the oldest and now-false sentence reading as current, which is what made a status report of the remaining work wrong |
| **Defects** Three carried, three closed | — | **Fixed 2026-09-03.** (1) **The suite's one intermittent failure.** The query log's per-cube locking test passed alone and failed under load, and it already had the capacity window the other measurements use --- which was not enough, because this machine has enough cores to *look* idle while sixty-four threads run on it. Eight parallel runs produced ratios of 0.54, 0.77 and 1.07 for code whose true ratio is above three; taking the best of five alternating rounds raised them to 1.08--1.64 and still failed. A measurement contending with a suite that exists to create contention cannot be estimated out of trouble, so it is `#[ignore]`d and run by `check-concurrency` beside the other four. (2) **`information_schema` ignored the projection.** Every catalogue query was answered with the whole row in this system's own order, whatever the client asked for, so `SELECT table_name FROM information_schema.tables` returned three columns with `table_schema` first --- and a driver reading `row[0]`, which is what a driver does, was handed the schema and told it was the table name. Not a wide answer, a **wrong** one with no error beside it. The projection is now read and the answer shaped to it, aliases included; anything unreadable --- `*`, a function, a cast --- falls back to the whole row, which is what every client got before, because a narrower guess would answer with a hole. One of the existing tests was asserting the defect and had to be corrected. (3) **The `MEAN`/`MAX` cube defect was already fixed and nothing pinned it.** A measure declared `MAX` returned the sum --- 15,687 where the maximum was 373.5 --- because the cell was read with a hardcoded summation one layer below where the model checks additivity; an adversarial review found it on 2026-09-01 and it was repaired. **The book went on documenting it as live for two days**, correctly by its own lights, because no test said otherwise --- wrong in the more damaging direction, telling people a working feature did not work. `tests/cube_rules.rs` now compares a cube declaring `MAX`, `MEAN` and `MIN` against `max()`, `avg()` and `min()` over the same rows on every build, with a mutation for the hardcoded sum. A defect that is fixed and unpinned is a defect that comes back, and until it does its obituary is the thing that is wrong |
| **M20** What changed between two versions | — | **Built 2026-09-03.** `SHOW CHANGES BETWEEN <a> AND <b> FOR <table>` --- commits, rows added, rows removed, files added, files removed, and compactions counted separately. Every number comes from the log and none opens a Parquet file, so it is answerable between two reporting runs on a table nobody would consider scanning. [ADR-0024](adr/0024-what-a-difference-between-two-versions-is.md) settles what a difference *is*, and the answer is why this took an afternoon after a day of being deferred: **a difference is a change to rows, and a compaction is not one** --- which the log has always said, because every add and remove carries `dataChange` and a compaction writes `false` on both sides. The deferral's objection was exactly right and its premise was not. Three decisions worth the ink: the range excludes the earlier version and includes the later, because the other reading is defensible and the two differ by one commit --- which is the difference between a reconciliation that ties out and one that does not; a compaction is **named** rather than omitted, because silence about it leaves a reader wondering why the storage looks nothing like it did; and there is deliberately no `rows_changed`, because an update here is a remove and an add and nothing says the two are the same row --- a guess would be right often enough to be trusted and wrong exactly when a key was rewritten, which is the case somebody is diffing to find. Six wire tests and two at the log, including the one the whole feature turns on: a compaction between two versions contributes nothing to the numbers **and is still reported**. Four mutations, all caught. Deferred by owner directive 2026-09-02, when `SHOW HISTORY OF` and `AS OF VERSION` were scoped into M17. A *tag* and a *diff* are different questions: the log records file-level adds and removes, so a compaction --- which rewrites files without changing a row --- would show as a total replacement. A diff that reports a compaction as a change is worse than no diff, because it looks like an answer. Needs an ADR deciding what a difference *is*, what it costs, and whether a compaction counts |
| **M18** Derived results | — | **In progress**, its first half built 2026-09-02 (M18a below). Closes [ADR-0014](adr/0014-materialized-views-and-the-cube-lifetime.md), Proposed since 2026-08-28: a maintained cube is already a materialized view in every respect that costs effort, and what it cannot express is a derived result that is **not an aggregate**. Also **user-supplied merge functions** (owner directive 2026-09-01): a measure may name a merge function --- Python first, then Rust, C++ and Java --- through ADR-0010's out-of-process contract. **The author guarantees associativity**, by owner decision; the server declares the rule, offers a diagnostic that composes in several orders and reports disagreement, and refuses rather than falling back to `Sum` when a merge fails |
| **M19** The data lifecycle policy | — | After M18. Owner directive 2026-09-01. One declaration governing how data ages across **both** tiers. The reframe that makes it safe: nothing moves --- capture already published it, so ageing rows out of the transactional store is a **release** gated on reconciliation's proof that the analytical copy exists. Detach, never delete; reversible for a grace period; and a read of released data is **refused by name** rather than answered short, which is the failure nearly every product ships |
...
### Step 1 — the lineage record, and who still reads a shared file

`sankhya-clone` holds the vocabulary the rest of `M10` consumes, built first because the clone
action, the maintenance wiring, the refusals and clone-aware backup all need it.

### Why the lineage is a table property and not a log action

The obvious home is a new action beside `add` and `remove`. It is the wrong one. This project's
open-storage claim is not a slogan --- there is a test asserting the Delta kernel reads these
logs --- and it is kept by writing only what the format defines. **An action nobody else knows is
a bet that every reader ignores what it does not recognise**, and losing that bet turns an open
table into one this software can read.

`Metadata.configuration` is what the format sets aside for exactly this. A clone's lineage is
three entries under a `sankhya.clone.` prefix, and a foreign reader sees a table with some
properties it does not use.

### Not a clone, and a clone whose lineage cannot be read

These are different answers and collapsing them is how the whole mechanism fails silently. A
table whose lineage is unreadable is one its origin's sweeper cannot know about, so answering
*"not a clone"* there is answering *"nothing else reads these files"* on no evidence. Three ways
a record can be broken are therefore refusals rather than absences: clone properties naming no
origin, an origin with no version, and a version that is not one.

A missing *timestamp* is not among them. The origin and the version are what a reclamation
decision needs; when the clone was taken is provenance, and refusing the whole record for want of
it would turn a cosmetic gap into a table whose origin cannot sweep.

### The set a sweep consults, and the wider one that is tempting

For a sweep of table `T`'s root, the tables that can still name a file there are `T` and
everything cloned from `T`, **transitively**. Ancestors are not in that set, and the narrowing
needs an argument: a clone's inherited files live under the origin's root, and the origin cannot
name a file the clone wrote after the split because it never heard of it.

"The whole family" is the intuitive answer and it is wider than its justification. Wider is
*safe* here --- it keeps files --- but a set nobody can reason about later is how a mechanism
stops being maintained, so `readers_of` and `family` are separate operations with separate names.
The wider one exists for the questions that need it: whether a table may be dropped, and what a
backup of a clone must include.

**A warehouse with no clones changes no reclamation decision.** `readers_of` returns the table
alone, and the sweep behaves exactly as it does today --- which is every table that exists.

### A cycle cannot happen, and is refused anyway

A clone's origin exists before the clone, so lineage is a tree by construction. It is a tree *in
the log*, which is a file, which somebody can edit. A resolver that trusted the construction
would hang rather than fail, and **a sweep that hangs stops every reclamation in the warehouse
until somebody notices** --- so the walk carries a visited set, reports the cycle, and contains
the refusal to the family it is in rather than stopping reclamation everywhere.

### A design question the ADR had not answered

Building this surfaced one: **how does a clone's log name a file it did not write?**
`AddFile.path` is *"relative to the table root, as the protocol requires"*, and the sweeper
compares names it made by stripping a table root. Both obvious answers are worse than they look.
An absolute URI is specification-legal and embeds a filesystem path, so **restoring into a
different directory silently produces a table whose files are all missing** --- and
restore-to-a-different-path is an operation this system has. A `../`-relative path is a bet on
every reader resolving `..` the same way, which the specification does not say.

So the clone's log names **none** of them. It records an origin and a version, and a read splices
the origin's live set *at that version* with the clone's own log --- not a new mechanism, since
[ADR-0015](adr/0015-the-shard-set-seam.md) found `plan_splice` already resolves one reference to
several sources and proves exact coverage. ADR-0016 is amended with this as Decision 1a.

**It costs the open-storage claim, for clones only, and that is the right trade.** A foreign
Delta reader pointed at a clone's directory sees the files the clone wrote and not the rows it
inherited. The alternative buys kernel-readable clones with absolute paths that break the moment
a warehouse is restored elsewhere --- trading a correctness property for an interoperability one.
A clone that must be read elsewhere is *materialised*, which is the explicit copy the ADR already
required for a clone that must travel without its origin.

### And it made the sweep's question smaller

Under Decision 1a the sweeper does not need the family walk at all. It asks *"which versions of
me does somebody still read?"*, which is a question about its own log, and the answer comes from
**direct clones only**: a clone of a clone pins a version of the intermediate table, whose files
are protected by the intermediate's own sweeper, and whose dependence on the root is expressed by
the intermediate's own pin. Transitivity does not compound, and the test that says so is the one
most likely to have been written the other way round.

The transitive walk stays, for the questions that genuinely span the tree: whether a table may be
dropped, and what a backup must include.
...
### Step 4 — the statement, and what it declines to read

`CREATE TABLE x CLONE y AT VERSION n` is not standard SQL, so the engine's parser rejects it
before any planning hook can see it and every extension point sits downstream of a successful
parse. It has to be recognised **before** the engine is asked --- the same conclusion
`sankhya-cube-sql` reached for `CREATE CUBE`, for the same reason.

That makes this a pre-filter on every statement the server receives, and **a pre-filter's
dangerous failure is not rejecting a clone. It is claiming something that was never its
business.** `CREATE TABLE orders (id BIGINT)` must reach the engine untouched, and so must
malformed SQL, whose error should come from the thing that owns the language rather than from
something that happened to look first. So most of the tests assert `None`: ordinary `CREATE
TABLE` in four shapes, statements that merely mention cloning, and four kinds of broken SQL.

Once `CLONE` is present the statement *is* ours, and then an error is better than silence ---
handing the engine something it will reject with a worse message helps nobody.

### `DROP TABLE` is deliberately not here

Dropping a table clones still read is one of the seven refusals, and the tempting place to
enforce it is a parser that intercepts `DROP TABLE`. **That would be wrong.** `DROP TABLE` is
ordinary SQL the engine owns, and a pre-filter claiming it would have to reimplement everything
the engine already does --- `IF EXISTS`, qualified names, all of it --- to hand back identical
behaviour in every case it does not care about.
...
### Two things carried rather than resolved

A clone with no `AT VERSION` carries `None`, not the newest version. *"Newest"* is a question
about a warehouse this module has never seen, and resolving it here would resolve it at **parse
time** --- a different moment from the one the clone is made at.

And keywords are not reserved: `CREATE TABLE clone CLONE entries` reads correctly, because a
table called `clone` is one somebody may already have and refusing it would be the parser
deciding what names a warehouse may use.

Fourteen tests and 5 mutations. Still not reachable from the server --- that is the next piece,
...
> **What `M24a` changed here, and what the measurement then said.** Until 2026-09-14 no
> `Date32` column had bounds at all, so Q3's `o_orderdate < …` — the predicate that makes it
> *the pruned shape* — skipped no files whatever, and the budget was met with the objective's
> own adjective doing no work.
>
> The gate now **reports** how many files that predicate proves irrelevant, because a budget
> met is not evidence that anything was pruned and only a number distinguishes the two states.
...
> a mechanism that prunes nothing predicts.
...
| **Q3** shipping priority — three-way join, top-N | 8 | 777 ms | **781 ms** | `NFR-PERF-03` pivot, < 1 s | **met**, pruning 0 of 23 files |
...
## Two controls that measured a share instead of the thing

**2026-08-31, both found the same way**: a concurrency criterion failed in the gate and passed
when run by hand, which is the shape of a measurement problem rather than a code one. The cause
in both is that `check-concurrency` builds its test binaries **optimized** and a plain
`cargo test` does not. With every participant faster, a writer holds a lock for less time, the
*serialized* arm does better, and any control expressed as a **ratio of the two arms** narrows
--- for a reason that has nothing to do with the path being measured.

**C2, the reader that must not wait.** The control was `flat >= serialized * 5.0`, a ratio of
throughput shares. What the criterion forbids is waiting, and waiting is a latency: the control
is now the lock-sharing reader's p99 against the free reader's, which separates by four orders
of magnitude --- 1.2s against 106µs --- where the share was a factor of a few.

**C1, the writers that must scale.** The control was `scales >= serialized * 2.0`. It is now
`serialized < FLOOR`: **the serialized arm must fail the very threshold the free arm passes.**
That reuses the number the test already demands rather than calibrating a second one, and it
states the property directly --- if a workload known to be serialized ever cleared the floor,
passing the floor would say nothing about whether writers are serialized.

In both cases the primary assertions are untouched. Only the controls changed, and both changed
from a proxy to the thing itself.

### C2 in full, as it was found

`read_latency_is_flat_under_write_load` --- the criterion that a reader is never blocked by a
writer --- failed in the gate and passed when run by hand. Both statements were true at once.
...
## The soak's first run with reconciliation switched on

**2026-09-01, 45 minutes, 20 GB across ten tables.** Every judged round reported `PASS` ---
resident memory, open files, metric series, warehouse bytes and live files all steady, with
`live_files` falling from 1080 to 90 as compaction caught up and holding there for the rest of
the run. 720 batches published, 13.6 GB scanned, 1.08 billion rows read, the cube answered
eighteen times, and the new ingest arm moved 14,400 documents: 13,680 published and 720
quarantined, which is exactly the one-in-twenty it writes deliberately.

**And it failed**, on reconciliation, which had never run before --- it was wired in on
2026-08-31 and this is the first soak since. Four tables reported missing rows, in round
numbers: 80,000, 160,000, 240,000.

The numbers are the clue. `expected` is merged the moment a batch is **absorbed**, and
absorbing hands it to the fan-out accumulator, which *defers* partitions still too small to be
worth a file. So the comparison was "what was handed over" against "what was written", and
those differ by design --- by whatever is sitting in the buffer at the instant of the scan.

**This is a defect in the harness, not in the product.** Reconciliation asks whether what was
written is still there; a row in a buffer has not been written. The check now flushes the
table's accumulator before scanning it, which is also what a deployment would do: you reconcile
against what you published.

Worth recording as a pattern rather than an incident. A check that has never run is not
evidence of anything, and its first run is as likely to find a defect in itself as in the
system --- which is why *"reconciliation ran and found nothing"* only became meaningful once
reconciliation had run.

---

## A projection over an empty table, which had never been run

**2026-08-31.** The first query ever aimed at a table with no live files *and* a column list
failed with an internal assertion: the scan returned an `EmptyExec` carrying the **whole**
schema, so the engine got a plan whose columns were not the ones it asked for.

Two things about how it was found are worth keeping. It was found by a **documentation
example** --- the guide's `SELECT source, position, reason_code, ... FROM sank_quarantine`,
which the gate insists on executing --- against a freshly created quarantine table. And every
existing test of that path had selected *every* column, which is the one projection under which
the bug cannot appear.

It is not a niche state: a freshly created table has no live files, and so does a table whose
files have all been retired. Both are ordinary, and both would have failed any projected query
...
cargo test --workspace           # 2907 tests, none of which needs a database
cargo xtask check-performance    # the NFR-PERF objectives, as a gate that can fail
python3 tools/mutation-audit.py  # 948 specific defects, applied one at a time
