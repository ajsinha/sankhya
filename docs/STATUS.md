...
| **M14** The client contract and the Python SDK | — | **In progress**, added 2026-08-31. **The SDK's examples became tests 2026-09-02** --- eight Python scripts under `sdk/python/examples/` and the eight SQL scripts under `sdk/sql/examples/`, both sets run in CI against a live server (`tests/sdk_examples.rs`, `tests/sql_examples.rs`); the SQL gate checks each statement's outcome in *both* directions, a `-- REFUSES` marker being required to fail and everything else to succeed. Writing them found four front-door defects a green gate had missed: `CREATE CUBE` could not name a table in a schema at all, a leading `--` comment made the server fail to recognise its own statements, `kept_by` named nothing, and the binding's `rollup`/`slice` passed the dimension where the measure goes while discarding their keyword options' names. Two shipped SQL examples contained statements that could never have run, one carrying a written note claiming it had been verified against a live server --- a `psql` script with `ON_ERROR_STOP off` prints its errors and keeps going, so a wall of output reads as success. Python first; Java and Rust in M16, which is why the *contract* matters more than the binding. **Transport security built 2026-08-31** --- one certificate, both doors, the wire protocol's own negotiation in front of it, and a startup line naming the posture in words. `FR-SEC-03`'s other half, federated identity, is still unbuilt: a `Principal` is a fixed tenant, and mutual TLS puts a client's certificate where a door can see it without anything yet deriving an identity from it. Also where [ADR-0010](adr/0010-external-aggregations.md) is built, its open question decided **out-of-process** by owner decision. The contract is decided in [ADR-0017](adr/0017-the-client-contract.md) and placed in [ARCHITECTURE](ARCHITECTURE.md) §11a. **All four server-side prerequisites built by 2026-09-02.** The fourth was the principal: `Handler::query` and `Handler::visible_tables` had no parameter for one, so `authenticate` read the user, refused an empty one on the grounds that an unattributable connection cannot be audited, and then **discarded it** --- every statement authorized as a literal and every audit entry attributed to it. A `Caller` now carries the connection's startup parameters inward, and the audit chain's head differs when the same statement is run by two users, which is the property `FR-SEC-02` asks for. **Authorization does not yet vary by subject** and that is stated rather than implied: this build gives every user the same role and tenant, so the identity now travels along a path that nothing distinguishes on. `FR-SEC-03`'s federated identity is what makes it observable. **Three of the four server-side prerequisites built by 2026-09-02** --- refusals cross as data (`code`, `sqlstate`, `remediation` and the `subjects` a refusal cites, in the protocol's hint field) and version skew is refused at connection with both versions named, a client declaring no contract being served as the generic driver it is. What remains is the principal: `Handler::query` has no parameter for one, so the authenticated user physically cannot travel inward. **Two of the four server-side prerequisites built 2026-09-01**: `SHOW LINEAGE OF <table>` and `SHOW DEPENDENTS OF <table>`, so a client can ask what a clone came from and what still reads a table *before* a drop refuses. Building them found that schema names were discarded at registration, that two tables of one name silently replaced each other, and that cloning had never worked against a table this server serves |
