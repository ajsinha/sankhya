...
| `FR-OLTP-02` | M | All data mutations SHALL be applied to the transactional store first. It is the sole authoritative writer of record. No other component may write to the published tier except the change applier and maintenance jobs |
...
| 5 | **DataFusion 55 disables filter pushdown and filter reordering by default** | `branch-55` configuration source: `pushdown_filters: default = false`, `reorder_filters: default = false` | Late materialization must be explicitly enabled **and asserted at startup**, not merely documented |
