...
| `FR-CUBE-10` | M | Cubes SHALL be **virtual by default** — computed from the fact source at query time. Materialisation of chosen levels MAY be declared explicitly, and a materialised level SHALL carry a staleness contract and SHALL be refreshable incrementally only where the measure forms a commutative monoid, as `FR-QUERY-27` requires |
| `FR-CUBE-11` | M | Materialisation SHALL never be automatic or implicit. A precomputed aggregate is a second copy that can disagree with its source, which is the cost this system exists to remove; paying it SHALL be a decision somebody makes and can find |
