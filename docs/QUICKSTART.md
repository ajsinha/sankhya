...
python3 tools/mutation-audit.py  # 256 deliberate defects, applied one at a time
...
applies 256 specific defects one at a time and requires the suite to fail on each. Twenty-nine
...
while — it is 256 sequential `cargo test` runs, and it edits your source files as it goes,
