...
```bash
cargo test --workspace          # 1,530 tests, none of which needs a database
```
...
python3 tools/mutation-audit.py  # 315 deliberate defects, applied one at a time
...
applies 315 specific defects one at a time and requires the suite to fail on each. Twenty-nine
...
while — it is 315 sequential `cargo test` runs, and it edits your source files as it goes,
