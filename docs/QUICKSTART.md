...
python3 tools/mutation-audit.py  # 301 deliberate defects, applied one at a time
...
applies 301 specific defects one at a time and requires the suite to fail on each. Twenty-nine
...
while — it is 301 sequential `cargo test` runs, and it edits your source files as it goes,
