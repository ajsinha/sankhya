...
python3 tools/mutation-audit.py  # 640 deliberate defects, applied one at a time
...
applies 640 specific defects one at a time and requires the suite to fail on each. Thirty-one
...
while — it is 640 sequential `cargo test` runs, and it edits your source files as it goes,
