...
python3 tools/mutation-audit.py  # 804 deliberate defects, applied one at a time
cargo xtask check-performance    # the NFR-PERF objectives, as a gate that can fail
SANKHYA_RELEASE=1 cargo xtask check-package   # the release artifact's platform baseline
```

**`check-all`** runs twenty-four invariants: the layer graph is acyclic and points the right
...
applies 804 specific defects one at a time and requires the suite to fail on each. Thirty-one
...
while — it is 804 sequential `cargo test` runs, and it edits your source files as it goes,
