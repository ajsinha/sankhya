...
python3 tools/mutation-audit.py  # 262 deliberate defects, applied one at a time
...
applies 262 specific defects one at a time and requires the suite to fail on each. Twenty-nine
...
while — it is 262 sequential `cargo test` runs, and it edits your source files as it goes,
