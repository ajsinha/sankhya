...
python3 tools/mutation-audit.py  # 365 deliberate defects, applied one at a time
...
applies 365 specific defects one at a time and requires the suite to fail on each. Twenty-nine
...
while — it is 365 sequential `cargo test` runs, and it edits your source files as it goes,
