...
python3 tools/mutation-audit.py  # 818 deliberate defects, applied one at a time
...
applies 818 specific defects one at a time and requires the suite to fail on each. Thirty-one
...
while — it is 818 sequential `cargo test` runs, and it edits your source files as it goes,
