...
python3 tools/mutation-audit.py  # 539 deliberate defects, applied one at a time
...
applies 539 specific defects one at a time and requires the suite to fail on each. Thirty-one
...
while — it is 539 sequential `cargo test` runs, and it edits your source files as it goes,
