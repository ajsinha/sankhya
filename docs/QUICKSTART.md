...
python3 tools/mutation-audit.py  # 483 deliberate defects, applied one at a time
...
applies 483 specific defects one at a time and requires the suite to fail on each. Thirty-one
...
while — it is 483 sequential `cargo test` runs, and it edits your source files as it goes,
