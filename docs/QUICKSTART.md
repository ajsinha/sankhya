...
python3 tools/mutation-audit.py  # 728 deliberate defects, applied one at a time
...
applies 728 specific defects one at a time and requires the suite to fail on each. Thirty-one
...
while — it is 728 sequential `cargo test` runs, and it edits your source files as it goes,
