...
python3 tools/mutation-audit.py  # 904 deliberate defects, applied one at a time
...
applies 904 specific defects one at a time and requires the suite to fail on each. Thirty-one
...
while — it is 904 sequential `cargo test` runs, and it edits your source files as it goes,
