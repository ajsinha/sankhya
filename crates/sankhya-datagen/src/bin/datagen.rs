...
use sankhya_datagen::{all_schemas, ColumnKind, Generator, Scale, Schema};
...
fn main() -> io::Result<()> {
    ...
    let gb = flag(&args, "--gb")
        ...
        .unwrap_or(1.0);
    let tables = flag(&args, "--tables")
        .and_then(|v| v.parse::<usize>().ok())
        .unwrap_or(10);
    let seed = flag(&args, "--seed")
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(0x5A4E_4B48_5941_0001);
...
fn emit_ddl(w: &mut impl Write, tables: usize) -> io::Result<()> {
    writeln!(
        w,
        "-- SANKHYA synthetic fixture schema. Generated; do not edit."
    )?;
    writeln!(
        w,
        "-- Deliberately non-financial: the general-purpose claim is tested, not asserted.\n"
    )?;
    for schema in all_schemas().iter().take(tables) {
        ...
        for (i, column) in schema.columns.iter().enumerate() {
            ...
            let pk = if matches!(column.kind, ColumnKind::Serial) {
                " PRIMARY KEY"
            } else {
                ""
            };
        ...
        for column in schema
            .columns
            .iter()
            .filter(|c| matches!(c.kind, ColumnKind::LargePayload { .. }))
        {
            writeln!(
                w,
...
fn emit_plan(w: &mut impl Write, generator: Generator, scale: Scale) -> io::Result<()> {
    writeln!(
        w,
        "{:<22} {:>14} {:>12} {:>14}",
        "table", "rows", "bytes/row", "approx bytes"
    )?;
    ...
    for (schema, rows) in &plan {
        ...
        writeln!(
            w,
            "{:<22} {rows:>14} {:>12} {bytes:>14}",
            schema.name,
            schema.approx_row_bytes()
        )?;
    }
    writeln!(
        w,
        "{:<22} {total_rows:>14} {:>12} {total_bytes:>14}",
        "TOTAL", ""
    )?;
    writeln!(
        w,
        "\n-- {:.2} GiB uncompressed across {} tables",
