...
use sankhya_datagen::{ColumnKind, Generator, Scale, WriteProfile, all_schemas, schema_by_name};
...
fn same_seed_reproduces_identical_rows() {
    ...
    assert_eq!(a.rows, b.rows, "the same seed must reproduce byte-identical rows");
}
...
fn different_seeds_diverge() {
    ...
    assert_ne!(a.rows, b.rows, "different seeds must produce different data");
}
...
fn rows_are_independently_reproducible() {
    ...
    assert_eq!(&whole.rows[40..48], &middle.rows[..], "row values must not depend on batch boundaries");
}
...
fn timestamps_increase_with_row_sequence() {
    ...
    let ts: Vec<&String> = batch
        .rows
        .iter()
        .filter_map(|r| r[6].as_ref())
        .collect();
    let mut sorted = ts.clone();
    sorted.sort();
    assert_eq!(ts, sorted, "timestamps must be non-decreasing with the row sequence");
}
...
fn large_payloads_resist_compression() {
    ...
    assert!(payload.len() >= 8_000, "payload is {} bytes, too small to go out-of-line", payload.len());
...
fn ten_tables_span_all_write_profiles() {
    ...
    for profile in [WriteProfile::AppendOnly, WriteProfile::SlowlyChanging, WriteProfile::HotMutable] {
        assert!(
...
fn no_schema_is_financial() {
    ...
    let forbidden = [
        "trade", "counterparty", "portfolio", "position", "notional",
        "ledger", "settlement", "isin", "cusip", "desk", "book",
    ];
    for schema in all_schemas() {
...
fn every_schema_has_a_partition_candidate() {
    ...
    for schema in all_schemas() {
        assert!(
            schema.columns.iter().any(|c| matches!(c.kind, ColumnKind::Date)),
...
fn the_acceptance_plan_is_sized_correctly() {
    ...
    let total: u64 = plan.iter().map(|(s, rows)| s.approx_row_bytes() * rows).sum();
    ...
    let widest = plan.iter().map(|(s, _)| s.approx_row_bytes()).max().unwrap_or(1);
    let narrowest = plan.iter().map(|(s, _)| s.approx_row_bytes()).min().unwrap_or(1);
    assert!(widest > narrowest, "the fixture set should vary in row width");
