...
use sankhya_ingest::{Reconciliation, RowDigest, TableDigest};
...
fn soak() {
    ...
    // What was written, per table, accumulated as it is written.
    //
    // The expectation has to come from the writer rather than from a second read of the
    // warehouse: a reconciliation that compared the published tier against itself would agree
    // with itself, which is the one answer it must not be able to give.
    let mut expected: Vec<TableDigest> = Vec::new();
    for table in 0..tables {
        let name = table_name(table);
        let root = at.join(&schema).join(&name);
        create_table(&root);
        let (written, digest) = fill(&root, per_table_bytes, &filling);
        println!(
            "{}    {name} filled, {:.2} GB, {} row(s) recorded for reconciliation",
            stamp(),
    ...
    // What reconciliation found, accumulated across the run.
    ...
    while started.elapsed() < deadline {
        ...
        for (index, root) in roots.iter().enumerate() {
            ...
            match append_one(accumulator, round) {
                Some(digest) => {
                    published += 1;
                    if let Some(running) = expected.get_mut(index) {
                        *running = running.merge(digest);
                    }
                }
                None => refused += 1,
            }
        ...
        // I/O than a round's other work, so doing it every round would make the soak a
        // measurement of this check; doing it never is how `Reconciliation::compare` came to
        // exist since `M9` with nothing outside its own crate calling it.
        //
        // **This is not `M11`.** That criterion is reconciliation running clean *in
        // production*, and this data was generated by the process checking it --- a closed
        // loop that cannot produce the source-side surprises the criterion exists to catch.
        // It is the sustained-soak evidence `M11`'s own section calls worth doing and
        ...
        if round % RECONCILE_EVERY == 0 {
            ...
            if let (Some(root), Some(want)) = (roots.get(which), expected.get(which)) {
                match observed_digest(root) {
                    Some(found) => {
                        reconciliation.compare(table_name(which), *want, found);
                        reconciled += 1;
                    }
                    None => {
                        // A table that cannot be read at all is not a clean reconciliation and
                        // must not be recorded as one. Counted separately so the report can say
                        // "reconciled 12, unreadable 1" rather than implying thirteen agreed.
                        unreadable += 1;
                    }
                }
            }
    ...
    println!(
        ...
        stamp(),
        reconciliation.rows_reconciled()
    );
    for (table, discrepancy) in reconciliation.discrepancies() {
        eprintln!("{}    RECONCILIATION `{table}`: {discrepancy}", stamp());
    }
    assert!(
        reconciliation.is_clean(),
        "the published tier no longer holds what was written: {:?}",
        reconciliation.discrepancies()
    );
    assert_eq!(
        unreadable, 0,
        "{unreadable} table(s) could not be read to reconcile at all, which is not a clean run"
    );
    ...
    assert!(
        ...
        "no reconciliation ran in {minutes} minute(s); at one every {RECONCILE_EVERY} rounds \
         this run was too short to check what it published"
    );
    println!(
...
/// The digest of a batch, over the columns a row *is*.
///
/// `event_date` is deliberately excluded. It is the partition column, and the published tier
/// stores it in the directory name rather than in every row --- so a digest that included it
/// would compare a value the writer holds against one the reader reconstructs, which is a
/// comparison of this harness against itself.
///
/// Rendered as text because that is what `RowDigest` takes, and because it is the rendering
/// both sides can agree on without sharing a decoder.
fn digest_of(batch: &RecordBatch) -> TableDigest {
    let mut digest = TableDigest::empty();
    let (Some(ids), Some(regions), Some(payloads)) = (
        batch.column_by_name("id").and_then(|c| c.as_any().downcast_ref::<Int64Array>()),
        batch.column_by_name("region").and_then(|c| c.as_any().downcast_ref::<StringArray>()),
    ...
    for row in 0..batch.num_rows() {
        ...
        let region = (!regions.is_null(row)).then(|| regions.value(row));
        let payload = payloads.value(row);
        digest.add_values(&[Some(id.as_str()), region, Some(payload)]);
    }
    digest
}

/// The digest of everything a table's log currently names.
///
/// # Why this is worth the scan
///
/// The soak's other measures answer *"is anything growing without bound?"*. This answers a
/// different question --- **is what was written still what is there?** --- and it is the one
/// `M11`'s criterion is about. Compaction rewrites files and retirement deletes them, and the
/// digest is invariant under both: a merge that lost a row, wrote one twice, or shifted a value
/// changes it, and nothing else in this harness would notice.
///
/// `Reconciliation::compare` has existed since `M9` with nothing outside its own crate calling
/// it. This is the caller.
fn observed_digest(root: &Path) -> Option<TableDigest> {
    let set = live_files(root).ok()?;
    let mut digest = TableDigest::empty();
    for file in &set.files {
        let handle = std::fs::File::open(root.join(&file.path)).ok()?;
        ...
        for batch in reader {
            let batch = batch.ok()?;
            // `TableDigest::merge` rather than arithmetic of this harness's own. The reconciler
            // states the property --- associative and commutative, so a digest may be computed
            // over arbitrary partitions and combined in any order --- and a second
            // implementation here would be a second thing to disagree with it.
            digest = digest.merge(digest_of(&batch));
        }
    }
    Some(digest)
}
...
fn fill(root: &Path, target_bytes: f64, since: &Instant) -> (u64, TableDigest) {
    ...
    while (written as f64) < target_bytes {
        ...
        let written_batch = batch(row, ROWS_PER_FILE);
        expected = expected.merge(digest_of(&written_batch));
...
fn append_one(accumulator: &mut Accumulator<'_>, sequence: u64) -> Option<TableDigest> {
    ...
    let written = batch_dated(from, 5_000, Dating::Arriving, newest);
    accumulator
        .absorb(&name, &written, Lsn::new(sequence.saturating_add(1)))
        .ok()
        .map(|_| digest_of(&written))
}
...
/// How many rounds between reconciliations.
///
/// A full table scan costs far more I/O than a round's other work, so reconciling every round
/// would make the soak a measurement of the check rather than of the system. Sixteen, so a
/// forty-five-minute run reconciles each of ten tables about once and a six-hour run does it
/// many times over --- and every table is covered, because the choice rotates.
const RECONCILE_EVERY: u64 = 16;
