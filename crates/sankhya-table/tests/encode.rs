...
use sankhya_table::{EncodeError, encode_batch};
...
fn row(values: Vec<Option<&str>>, lsn: u64) -> Mutation {
    Mutation {
        relation_id: 1,
        op: Op::Insert,
        row: Row { values: values.into_iter().map(|v| v.map(str::to_string)).collect() },
...
fn a_batch_carries_provenance_columns() {
    ...
    let batch = encode_batch(&s, &[row(vec![Some("1")], 100), row(vec![Some("2")], 200)])
        .expect("encodes");

    assert_eq!(batch.num_rows(), 2);
    assert_eq!(batch.num_columns(), 4, "one declared column plus three provenance columns");

    let arrow_schema = batch.schema();
    let names: Vec<&str> = arrow_schema.fields().iter().map(|f| f.name().as_str()).collect();
    assert_eq!(names, ["id", "_sankhya_commit_lsn", "_sankhya_commit_ts", "_sankhya_op"]);
}
...
fn the_commit_position_travels_with_every_row() {
    ...
    let s = schema(vec![("id", LogicalType::Int64, false)]);
    let batch = encode_batch(&s, &[row(vec![Some("1")], 4242), row(vec![Some("2")], 4242)])
        .expect("encodes");
    let lsn = batch
        .column(1)
        .as_any()
...
fn nulls_are_preserved_where_the_column_permits_them() {
    ...
    let batch = encode_batch(&s, &[row(vec![None], 1), row(vec![Some("present")], 2)])
        .expect("encodes");
    let column = batch.column(0).as_any().downcast_ref::<StringArray>().expect("strings");
...
fn zoned_timestamps_keep_their_zone() {
    ...
    let column = batch
        ...
        .expect("timestamps");
    assert_eq!(column.value(0), 1_735_689_600_000_000, "2025-01-01T00:00:00Z in microseconds");
...
fn dates_and_timestamps_agree_on_the_epoch() {
    ...
    let batch = encode_batch(&s, &[row(vec![Some("1970-01-01"), Some("1970-01-01 00:00:00+00")], 1)])
        .expect("encodes");
    let d = batch.column(0).as_any().downcast_ref::<arrow_array::Date32Array>().expect("dates");
    let t = batch
        .column(1)
        .as_any()
        .downcast_ref::<TimestampMicrosecondArray>()
...
fn the_operation_is_recorded_per_row() {
    ...
    let ops = batch.column(3).as_any().downcast_ref::<StringArray>().expect("strings");
...
fn integers_round_trip_at_the_boundaries() {
    ...
    .expect("encodes");
    let column = batch.column(0).as_any().downcast_ref::<Int64Array>().expect("ints");
