...
What works today: a verified dependency set that compiles, a `pgoutput` wire decoder validated against a real PostgreSQL 17.11 stream, an apply path whose transaction invariant is property-tested, a lossless type mapping checked against a real schema, mirror naming that refuses collisions rather than disambiguating them, a read-path splice planner proven to cover a query's span exactly once, and a complete vertical slice in which a captured workload becomes Parquet and answers SQL with an exact decimal sum matching the source's own arithmetic. What does not: continuous multi-table operation, the catalog, the graph engine, the API surfaces, the server itself. [`docs/QUICKSTART.md`](docs/QUICKSTART.md) is explicit about the boundary.
