...
CATALOGUE = [
    ...
    ("plan: report a coverage gap as a freshness problem",
     ...
     "            SpliceError::CoverageGap { from, to } => {\n                sankhya_error::Error::StaleData(format!(\"no tier covers ({from}, {to}]\"))",
     "sankhya-plan", 1, "splice"),

    ("plan: page for a query that merely arrived early",
     "crates/sankhya-plan/src/splice.rs",
     "            } => sankhya_error::Error::StaleData(format!(",
     "            } => sankhya_error::Error::CoverageGap(format!(",
     "sankhya-plan", 1, "splice"),
    ...
    ("tiering: hand out a witness for a table the reconciliation refused",
     "crates/sankhya-tiering/src/registry.rs",
     "        self.servable(table)\n            .ok_or_else(|| crate::unify::Unservable::NotReconciled {",
     "        Some(Servable { table: table.to_string() })\n            .ok_or_else(|| crate::unify::Unservable::NotReconciled {",
     "sankhya-tiering", 1, "unification"),
